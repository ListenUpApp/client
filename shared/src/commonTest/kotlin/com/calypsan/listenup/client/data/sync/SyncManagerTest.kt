...
class SyncManagerTest {
    ...
    private class TestFixture(private val scope: CoroutineScope) {
        ...
        fun build(): SyncManager = SyncManager(
            syncApi = syncApi,
            bookDao = bookDao,
            seriesDao = seriesDao,
    ...
    private fun stubSuccessfulSync(fixture: TestFixture) {
        ...
        everySuspend { fixture.syncApi.getBooks(any(), any(), any()) } returns Result.Success(
            SyncBooksResponse(
                books = emptyList(),
        ...
        everySuspend { fixture.syncApi.getSeries(any(), any(), any()) } returns Result.Success(
            SyncSeriesResponse(
                series = emptyList(),
        ...
        everySuspend { fixture.syncApi.getContributors(any(), any(), any()) } returns Result.Success(
    ...
    @Test
    fun `sync returns success when all operations complete`() = runTest {
        // Given
    ...
    @Test
    fun `sync transitions state from Idle to Syncing to Success`() = runTest {
        // Given
        val fixture = createFixture()
        val syncManager = fixture.build()
        advanceUntilIdle()
    ...
    @Test
    fun `sync updates last sync time on success`() = runTest {
        // Given
        val fixture = createFixture()
        val syncManager = fixture.build()
        advanceUntilIdle()
    ...
    @Test
    fun `sync rebuilds FTS tables on success`() = runTest {
        // Given
        val fixture = createFixture()
        val syncManager = fixture.build()
        advanceUntilIdle()
    ...
    @Test
    fun `sync connects to SSE stream on success`() = runTest {
        // Given
        val fixture = createFixture()
        val syncManager = fixture.build()
        advanceUntilIdle()
    ...
    @Test
    fun `sync continues when FTS rebuild fails`() = runTest {
    ...
    @Test
    fun `sync returns failure when API fails after retries`() = runTest {
        // Given
        val fixture = createFixture()
    ...
    @Test
    fun `sync transitions to Error state on failure`() = runTest {
        // Given
        val fixture = createFixture()
        val error = RuntimeException("Network error")
        everySuspend { fixture.syncApi.getBooks(any(), any(), any()) } returns Result.Failure(error)
    ...
    @Test
    fun `sync rethrows CancellationException`() = runTest {
        // Given
        val fixture = createFixture()
    ...
    @Test
    fun `sync transitions to Idle on cancellation`() = runTest {
        // Given
        val fixture = createFixture()
        everySuspend { fixture.syncApi.getBooks(any(), any(), any()) } throws CancellationException("Cancelled")
        val syncManager = fixture.build()
    ...
    @Test
    fun `sync succeeds on retry after transient failure`() = runTest {
    ...
    @Test
    fun `sync performs full sync when no last sync time`() = runTest {
        // Given
        val fixture = createFixture()
        everySuspend { fixture.syncDao.getValue(SyncDao.KEY_LAST_SYNC_BOOKS) } returns null
        val syncManager = fixture.build()
    ...
    @Test
    fun `sync performs delta sync when last sync time exists`() = runTest {
        // Given
    ...
    @Test
    fun `forceFullSync clears last sync time before syncing`() = runTest {
        // Given
        val fixture = createFixture()
    ...
    @Test
    fun `forceFullSync returns sync result`() = runTest {
        // Given
        val fixture = createFixture()
        everySuspend { fixture.syncDao.delete(SyncDao.KEY_LAST_SYNC_BOOKS) } returns Unit
        val syncManager = fixture.build()
    ...
    @Test
    fun `sync handles pagination for books`() = runTest {
    ...
    @Test
    fun `sync handles book deletions from server`() = runTest {
    ...
    @Test
    fun `sync handles series deletions from server`() = runTest {
        // Given
        val fixture = createFixture()
        everySuspend { fixture.syncApi.getSeries(any(), any(), any()) } returns Result.Success(
            SyncSeriesResponse(
    ...
    @Test
    fun `sync handles contributor deletions from server`() = runTest {
        // Given
        val fixture = createFixture()
        everySuspend { fixture.syncApi.getContributors(any(), any(), any()) } returns Result.Success(
            SyncContributorsResponse(
    ...
    @Test
    fun `sync marks conflict when server has newer version than local unsynced changes`() = runTest {
        // Given - Local book is NOT_SYNCED and server has newer version
        val fixture = createFixture()
        val localBook = createBookEntity(
            id = "book-1",
