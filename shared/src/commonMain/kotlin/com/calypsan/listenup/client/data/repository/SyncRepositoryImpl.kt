 ...
 */
private fun DataSyncStatus.toDomain(): SyncState = when (this) {
    is DataSyncStatus.Idle -> SyncState.Idle
    is DataSyncStatus.Syncing -> SyncState.Syncing
    is DataSyncStatus.Progress -> SyncState.Progress(
        phase = phase.toDomain(),
        current = current,
        total = total,
        message = message,
    )
    is DataSyncStatus.Retrying -> SyncState.Retrying(
        attempt = attempt,
        maxAttempts = maxAttempts,
    )
    is DataSyncStatus.Success -> SyncState.Success(
        timestamp = timestamp,
    )
    is DataSyncStatus.Error -> SyncState.Error(
        message = exception.message ?: "Sync failed",
        exception = exception,
    )
    is DataSyncStatus.LibraryMismatch -> SyncState.LibraryMismatch(
 ...
 */
private fun DataSyncPhase.toDomain(): DomainSyncPhase = when (this) {
